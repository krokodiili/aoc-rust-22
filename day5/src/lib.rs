...
fn strip_string_to_alphabets_tabs(stack_string: String) -> String {
    ...
    let result: Vec<&str> = re
        .find_iter(&stack_string)
        .map(|cap| return cap.as_str())
        .collect();
...
mod tests {
    ...
    fn execute_move_works() {
        ...
        let result = execute_move(starting_point, command, Some(false));
